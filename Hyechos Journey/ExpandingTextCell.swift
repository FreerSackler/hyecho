...
class ExpandingTextCell: UICollectionViewCell {
    ...
    var time: Double {
        ...
        set {
            ...
            timeLabel.text = "-" + (audioPlayer!.duration - time).timeString
    ...
    func updateProgress() {
       ...
       // print(Float(progress))
        ...
        timeLabel.text = "-" + (totalTime - audioPlayer!.currentTime).timeString
        
        
        //Temp fix for random audio pauses. Could not find underlying reason for it pausing...
        if !audioPlayer!.isPlaying && startedPlaying {
            audioPlayer!.play()
        }
    }
    ...
    @IBAction func didPressPlayPause(_ sender: UIButton) {
        ...
        if paused {
            UIView.transition(with: playPauseButton, duration: 0.25, options: .transitionFlipFromLeft, animations: {
                self.playPauseButton.setImage(ExpandingTextCell.play, for: .normal)
            }, completion: nil)
        } else {
            UIView.transition(with: playPauseButton, duration: 0.25, options: .transitionFlipFromRight, animations: {
                self.playPauseButton.setImage(ExpandingTextCell.pause, for: .normal)
            }, completion: nil)
        }
        if audioPlayer == nil {
            createAudioPlayer()
...
class ExpandingTextCellPoem: ExpandingTextCell {
